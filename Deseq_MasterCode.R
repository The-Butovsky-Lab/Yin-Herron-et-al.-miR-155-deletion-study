...
Location_of_quant_Data_Files <-"/Users/dandoone/Dropbox (Partners HealthCare)/SHIP1cKO_MGnD/Experiment 1/data"  #  <--- Find the location of the folder (sometimes called transcripts_quant) that contains folders for all samples with quant.sf file in them
Name_of_Metadata_File <- "metadata_phag.xlsx"  # name of your metadata xlsx file. 
...
Metadata1_Column_Name <- "Genotype2"       #  <--- The more significant comparison (example: APP vs WT) 

# ____________Variable 2 ONLY for multiple comparison (if you have 2 or 3 different variables to compare) ____________
Metadata2_Column_Name <- "Genotype"   #  <--- The sub-comparison/secondary comparison (example:  APOE3 vs APOE4)

#  ____________Variable 3 ONLY for multiple comparison (if you have 3 different variables to compare)____________
Metadata3_Column_Name <- "cre"         #  <--- The last-comparison/most "sub" comparison (example:  KI vs KO)
...
Select_Genes_in_Data <- "Yes"         #  <--- Are there different genders in your data? "Yes" or "No"
...
Statistics <- "pvalue"               # <---- either "pvalue" or "padj"
alpha <- 0.01                        # <---- alpha cutoff for statistical signifigance 
...
Do_you_want_barplots_for_select_genes <- "Yes"      # <----  "Yes" or "No"
...
Z1 <- FALSE # <- If order of metadata 1 in your figures is flipped, change (from TRUE to FALSE) or  vice versa
...
cts_subset <- cts_final %>% head(n = 100)
...
top100<- t(apply(round(cts_subset), 1, cal_z_score))
heatmap100<- pheatmap(top100,
                    color = colorRampPalette(c("navy", "white", "firebrick3"))(48),
                    breaks = seq(-2, 2, by = 0.1),
                    kmeans_k = NA,
                    cluster_rows = T,
...
pdf(file = paste0("plots/heatmap_",filenames,"_comparison","_top100.pdf"), pointsize = 10, width = 7, height =11)
print(heatmap100)
dev.off()
}
